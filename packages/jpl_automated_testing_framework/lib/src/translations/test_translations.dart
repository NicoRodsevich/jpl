import 'package:jpl_static_translations/static_translations.dart';

class TestTranslations {
  TestTranslations._();

  static const atf_added_step_action = TranslationEntry(
    key: 'atf_added_step_action',
    value: 'Added Step: {step}',
  );

  static const atf_available_test_steps = TranslationEntry(
    key: 'atf_available_test_steps',
    value: 'Available Test Steps',
  );

  static const atf_bdd = TranslationEntry(
    key: 'atf_bdd',
    value: 'BDD',
  );

  static const atf_button_cancel = TranslationEntry(
    key: 'atf_button_cancel',
    value: 'CANCEL',
  );

  static const atf_button_clear = TranslationEntry(
    key: 'atf_button_clear',
    value: 'CLEAR',
  );

  static const atf_button_close = TranslationEntry(
    key: 'atf_button_close',
    value: 'CLOSE',
  );

  static const atf_button_discard = TranslationEntry(
    key: 'atf_button_discard',
    value: 'DISCARD',
  );

  static const atf_button_delete = TranslationEntry(
    key: 'atf_button_delete',
    value: 'DELETE',
  );

  static const atf_button_edit = TranslationEntry(
    key: 'atf_button_edit',
    value: 'EDIT',
  );

  static const atf_button_export = TranslationEntry(
    key: 'atf_button_export',
    value: 'EXPORT',
  );

  static const atf_button_export_golden_images = TranslationEntry(
    key: 'atf_button_export_golden_images',
    value: 'EXPORT AS GOLDEN IMAGES',
  );

  static const atf_button_ok = TranslationEntry(
    key: 'atf_button_ok',
    value: 'OK',
  );

  static const atf_button_rerun = TranslationEntry(
    key: 'atf_button_rerun',
    value: 'RUN TEST AGAIN',
  );

  static const atf_button_run = TranslationEntry(
    key: 'atf_button_run',
    value: 'RUN',
  );

  static const atf_button_run_all = TranslationEntry(
    key: 'atf_button_run_all',
    value: 'RUN ALL',
  );

  static const atf_button_submit = TranslationEntry(
    key: 'atf_button_submit',
    value: 'SUBMIT',
  );

  static const atf_button_view_steps = TranslationEntry(
    key: 'atf_button_view_steps',
    value: 'VIEW STEPS',
  );

  static const atf_button_view_test_steps = TranslationEntry(
    key: 'atf_button_view_test_steps',
    value: 'VIEW TEST STEPS ({count})',
  );

  static const atf_clear_confirmation = TranslationEntry(
    key: 'atf_clear_confirmation',
    value: 'Are you sure you want to clear all the test steps?',
  );

  static const atf_copied_to_clipboard = TranslationEntry(
    key: 'atf_copied_to_clipboard',
    value: 'Copied to clipboard',
  );

  static const atf_correct_errors = TranslationEntry(
    key: 'atf_correct_errors',
    value: 'Please correct form errors before continuing.',
  );

  static const atf_custom_pages = TranslationEntry(
    key: 'atf_custom_pages',
    value: 'Custom Pages',
  );

  static const atf_discard_changes = TranslationEntry(
    key: 'atf_discard_changes',
    value: 'Discard changes?',
  );

  static const atf_error_has_occurred = TranslationEntry(
    key: 'atf_error_has_occurred',
    value: 'An error has occurred',
  );

  static const atf_export_successful = TranslationEntry(
    key: 'atf_export_successful',
    value: 'Export successful',
  );

  static const atf_full = TranslationEntry(
    key: 'atf_full',
    value: 'FULL',
  );

  static const atf_maximize_step_details = TranslationEntry(
    key: 'atf_maximize_step_details',
    value: 'Show full step details',
  );

  static const atf_minified = TranslationEntry(
    key: 'atf_minified',
    value: 'MINIFIED',
  );

  static const atf_minimize_step_details = TranslationEntry(
    key: 'atf_minimize_step_details',
    value: 'Show minimal step details',
  );

  static const atf_no_filter = TranslationEntry(
    key: 'atf_no_filter',
    value: '<No Filter>',
  );

  static const atf_no_test_step = TranslationEntry(
    key: 'atf_no_test_step',
    value: '<No Test Step>',
  );

  static const atf_no_tests_found = TranslationEntry(
    key: 'atf_no_tests_found',
    value: 'No tests found',
  );

  static const atf_open_tests_page = TranslationEntry(
    key: 'atf_open_tests_page',
    value: 'Open Tests Page',
  );

  static const atf_operation_not_supported = TranslationEntry(
    key: 'atf_operation_not_supported',
    value: 'Operation not supported',
  );

  static const atf_options = TranslationEntry(
    key: 'atf_options',
    value: 'Options',
  );

  static const atf_pin_step = TranslationEntry(
    key: 'atf_pin_step',
    value: 'PIN STEP',
  );

  static const atf_quick_add = TranslationEntry(
    key: 'atf_quick_add',
    value: 'Quick Add Step',
  );

  static const atf_select_test_suite = TranslationEntry(
    key: 'atf_select_test_suite',
    value: 'Select Test Suite',
  );

  static const atf_selected_widget_steps = TranslationEntry(
    key: 'atf_selected_widget_steps',
    value: 'Selected Widget Steps',
  );

  static const atf_show_global_overlays = TranslationEntry(
    key: 'atf_show_global_overlays',
    value: 'Show global overlays',
  );

  static const atf_step_currently_pinned = TranslationEntry(
    key: 'atf_step_currently_pinned',
    value: 'STEP CURRENTLY PINNED',
  );

  static const atf_suite_name = TranslationEntry(
    key: 'atf_suite_name',
    value: 'Suite Name',
  );

  static const atf_suite_version_steps = TranslationEntry(
    key: 'atf_suite_version_steps',
    value: 'Suite: {suiteName} / Version: {version} / Steps: {steps}',
  );

  static const atf_test_name = TranslationEntry(
    key: 'atf_test_name',
    value: 'Test Name',
  );

  static const atf_test_options = TranslationEntry(
    key: 'atf_test_options',
    value: 'Test Options',
  );

  static const atf_test_steps = TranslationEntry(
    key: 'atf_test_steps',
    value: 'Test Steps',
  );

  static const atf_test_suite_results = TranslationEntry(
    key: 'atf_test_suite_results',
    value: 'Test Suite Results',
  );

  static const atf_test_results = TranslationEntry(
    key: 'atf_test_results',
    value: 'Test Results',
  );

  static const atf_tests = TranslationEntry(
    key: 'atf_tests',
    value: 'Tests',
  );

  static const atf_timeout_seconds = TranslationEntry(
    key: 'atf_timeout_seconds',
    value: 'Timeout (seconds)',
  );

  static const atf_tooltip_add_and_run = TranslationEntry(
    key: 'atf_tooltip_add_and_run',
    value: 'Add step and immediately execute',
  );

  static const atf_tooltip_copy_bdd = TranslationEntry(
    key: 'atf_tooltip_copy_bdd',
    value: 'COPY BDD TEXT',
  );

  static const atf_tooltip_copy_steps = TranslationEntry(
    key: 'atf_tooltip_copy_steps',
    value: 'COPY STEPS',
  );

  static const atf_tooltip_move_down = TranslationEntry(
    key: 'atf_tooltip_move_down',
    value: 'MOVE DOWN',
  );

  static const atf_tooltip_move_up = TranslationEntry(
    key: 'atf_tooltip_move_up',
    value: 'MOVE UP',
  );

  static const atf_unnamed_test = TranslationEntry(
    key: 'atf_unnamed_test',
    value: '<Unnamed Test>',
  );

  static const atf_version = TranslationEntry(
    key: 'atf_version',
    value: 'Version',
  );

  static const atf_version_steps = TranslationEntry(
    key: 'atf_version_steps',
    value: 'Version: {version} / Steps: {steps}',
  );

  static const atf_widget = TranslationEntry(
    key: 'atf_widget',
    value: 'Widget',
  );

  static const atf_widgetless_steps = TranslationEntry(
    key: 'atf_widgetless_steps',
    value: 'Other Steps',
  );
}
