  ...
  static TestStepDelays fromDynamic(dynamic map) {
    ...
    if (map == null) {
      ...
      result = TestStepDelays(
        defaultTimeout: JsonClass.maybeParseDurationFromMillis(
              map['defaultTimeout'],
            ) ??
            const Duration(seconds: 10),
        postFoundWidget: JsonClass.maybeParseDurationFromMillis(
              map['postFoundWidget'],
            ) ??
            Duration.zero,
        postStep: JsonClass.maybeParseDurationFromMillis(
              map['postStep'],
            ) ??
            const Duration(seconds: 1),
        postSubmitReport: JsonClass.maybeParseDurationFromMillis(
              map['postSubmitReport'],
            ) ??
            const Duration(seconds: 5),
        preStep: JsonClass.maybeParseDurationFromMillis(
              map['preStep'],
            ) ??
            const Duration(seconds: 1),
        screenshot: JsonClass.maybeParseDurationFromMillis(
              map['screenshot'],
            ) ??
            const Duration(seconds: 5),
        scrollIncrement: JsonClass.maybeParseDurationFromMillis(
              map['scrollIncrement'],
            ) ??
            const Duration(milliseconds: 100),
        testSetUp: JsonClass.maybeParseDurationFromMillis(
              map['testSetUp'],
            ) ??
            const Duration(milliseconds: 500),
        testTearDown: JsonClass.maybeParseDurationFromMillis(
              map['testTearDown'],
            ) ??
            const Duration(milliseconds: 500),
      );
