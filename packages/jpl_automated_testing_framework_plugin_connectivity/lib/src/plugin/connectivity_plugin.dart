...
class ConnectivityPlugin with WidgetsBindingObserver {
        ...
        runZonedGuarded(() {
          _connectivitySubscription =
            ...
            (List<ConnectivityResult> results) =>
                _updateData(_getConnected(_reduceResults(results))),
          );
        }, (_, __) {
          // no-op
        });
  ...
  /// Reduces the list of connectivity results returned by `connectivity_plus`
  /// into a single [ConnectivityResult].  Returns the first non-[none] result
  /// when present, otherwise [ConnectivityResult.none].
  ...
  Future<ConnectivityResult> _getConnectivityResult() async {
    final results = await runZonedGuarded<Future<List<ConnectivityResult>>>(
    ...
    return results == null ? ConnectivityResult.wifi : _reduceResults(results);
  }
