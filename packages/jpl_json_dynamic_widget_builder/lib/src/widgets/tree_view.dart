...
import 'package:jpl_json_dynamic_widget_builder/src/widgets/widget_properties_editor.dart';
...
class _TreeViewState extends State<TreeView>
  ...
  Widget build(BuildContext context) {
    // `numSupportedChildren` was removed from the public `JsonWidgetBuilder`
    // API in jpl_json_dynamic_widget v12.  Treat a selected widget as
    // supporting an unlimited number of children (-1), matching the legacy
    // default behavior, and 0 when nothing is selected.
    final numSupportedChildren = _widgetTreeBloc.current == null ? 0 : -1;
                                  ...
                                  child: ElevatedButton.icon(
                                    ...
                                    onPressed: () {
                                      ...
                                      if (parent == null) {
                                        ...
                                        final remaining = (parent.children ??
                                                <JsonWidgetData>[])
                                            .where((child) =>
                                                child.id !=
                                                _widgetTreeBloc.current!.id)
                                            .toList();
                                        final newParent = parent.copyWithCompat(
                                          children: remaining,
                                        );
                                        _widgetTreeBloc.widget =
                                            _widgetTreeBloc.replace(
                                          parent,
                                          newParent,
                                        );
                                      }
